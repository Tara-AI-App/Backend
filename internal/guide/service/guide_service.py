...
class GuideService:
    ...
    async def get_guide_by_id(self, guide_id: UUID, user_id: UUID) -> GuideDetailResponse:
        ...
        try:
            logger.info(f"Getting guide {guide_id} for user {user_id}")
            return await self.guide_repository.get_guide_by_id(guide_id, user_id)
        except ValueError as e:
            # Re-raise ValueError as is (guide not found)
            raise e
        except Exception as e:
            logger.error(f"Error getting guide {guide_id} for user {user_id}: {str(e)}")
            raise e
